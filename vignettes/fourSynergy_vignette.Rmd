...
(<https://github.com/sophiewind/fourSynergy>\_pipeline) and can be
executed using:

```{bash snakemake, eval=FALSE}
snakemake \
    -s Snakefile_dyn \
    --cores 30 \
...
```{r plot_ia, message=FALSE, eval=TRUE}
...
```{r plot_tracks_base, message=FALSE, eval=TRUE}
plotBaseTracks(sia)
```
...
```{r plot_ens_tracks, message=FALSE, eval=TRUE}
