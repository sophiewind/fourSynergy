...
plotDiffIa <- function(ia, genes_of_interest = NULL, cex.chr = 1,
  ...
  if(plot_spider == TRUE){
    ...
    start.regs <- rep(start.reg, length(ia@expConsensus[ia@expConsensus$significance > 0]))
    kpPlotLinks(kp, start.regs, ia@expConsensus[ia@expConsensus$significance > 0],
                col = "firebrick4",r0 = 0.35, r1 = 0.39)
    start.regs_ctrl <- rep(start.reg, length(ia@ctrlConsensus[ia@ctrlConsensus$significance > 0]))
    kpPlotLinks(kp, start.regs_ctrl, ia@ctrlConsensus[ia@ctrlConsensus$significance > 0],
