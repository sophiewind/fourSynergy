...
plotTracks <- function(ia, kp, bgs, r0 = 0, r1 = 1, cex.vp = 1,
    ...
    for (bg in names(bgs)) {
        if (grepl(ia@metadata$condition, bg)) {
            kpPlotDensity(kp,
                data.panel = 1, bgs[[bg]],
                col = adjustcolor("firebrick4", alpha.f = 1 / length(bgs) * 2),
                r0 = (r0 + r1) / 2, r1 = r1, window.size = 10000
            )
        } else {
            kpPlotDensity(kp,
                data.panel = 1, bgs[[bg]],
                col = adjustcolor("darkblue", alpha.f = 1 / length(bgs) * 2),
    ...
    ticks_to_use <- seq(-max_count, max_count, by = max_count / 5)
    labels_to_show <- pretty(abs(ticks_to_use))
    kpAxis(kp,
        r0 = 0.5, r1 = 1, ymin = -max_count,
        ymax = max_count, numticks = 5, labels = labels_to_show,
        cex = cex.y.track, side = 1
    )
}
