...
plotTracks <- function(ia, kp, bgs, r0 = 0, r1 = 1, cex.vp = 1,
  ...
  kpAxis(kp,
         r0 = r0, r1 = 1, ymin = -plot_max,
         ymax = plot_max, numticks = 5, labels = labels_to_show,
  ...
  if (length(genes_of_interest) == 1 && genes_of_interest == "all") {
    ...
    for (i in sg$name) {
      ...
      zr <- GRanges(
        seqnames = unique(as.character(seqnames(gene))),
        ranges = IRanges(
          start = max(1, start(gene)) + ((end(gene) - max(1, start(gene)))/2),
          end = max(1, start(gene)) + ((end(gene) - max(1, start(gene)))/2) + 1
        ),
        strand = strand(gene)
      )
      pdf(NULL)
